ResultsPcaUI <- function(id){
  ...
  fluidPage(
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          div(id = ns("Data-Stuff"),
            #Number Of Items start at 500 (default) go to num of rows----
            div(
              p("PCA Elements ", style = "margin-right: 5%; padding-bottom: 2%; width: 80%"),
              p("Number of elements -by variance- that are factored into the PCA. (The default for the DESeq2 package is 500)",
    ...
    mainPanel(
      ...
      div(
        withSpinner(
          plotOutput(ns("PCA"), width = "100%", height = "450px"),
          type = 6
        )
      )
